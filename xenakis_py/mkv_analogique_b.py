"""
Analogique B Sine Wave Generator
-------------------------------
Generates a sequence of screen states using MarkovChain (Chapter III, pp. 79–109)
Each screen is rendered as a stochastic cloud of MIDI events, but all events are mapped to pure sine wave output.
Output: analogique_b_demo.mid
...
# MIDI channel and program for sine wave (usually Synth Lead or Pad, but true sine is best handled by a softsynth)
SINE_CHANNEL = 0
SINE_PROGRAM = 81  # General MIDI: Lead 2 (sawtooth) or 88 (pad), but for true sine, use a synth that interprets this as sine

renderer.set_program(SINE_CHANNEL, SINE_PROGRAM)
...
if __name__ == "__main__":
    ...
    # Save with timestamp
    filename = f"analogique_b_demo_{ts}.mid"
